...
function line(start, state, dispatch) {
    function drawLine(pos) {
        ...
        let xStart = start.x; //Math.min(start.x, pos.x);
        let yStart = start.y //Math.min(start.y, pos.y);
        let xEnd = pos.x; //Math.max(start.x, pos.x);
        let yEnd = pos.y // Math.max(start.y, pos.y);

        let A = Math.round((yEnd - yStart) / (xEnd - xStart));


        let B = Math.round(yStart - A * xStart);
...
const startState = {
    tool: "draw",
    color: "#000000",
    picture: Picture.empty(200, 200, "#f0f0f0"),
    done: [],
    doneAt: 0
};
const baseTools = {
